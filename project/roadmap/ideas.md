...
| ✅ | Docs & roadmap refresh | Added architecture/config/privacy/evaluation docs and aligned README streaming guidance. | Shipped in PR 17. Owner: @natanayalo. Rollback: revert PR. Demo: n/a. |
...
| ✅ | Preference expansion (distance filter) | Respect user travel radius when ranking providers and expose the applied limit in planner reasons. | Shipped in PR 35. Owner: @natanayalo. Rollback: `PREFERENCE_TRAVEL_LIMIT_ENABLED=false`. Demo: `curl -s http://localhost:8000/api/graph/run -d '{"user_id":"demo","query":"Find a clinic nearby","preferences":{"max_travel_km":5}}' | jq '.state.candidates | map({name, distance_km, reasons})'`. |
