...
def _base_name(name: str) -> str:
    return re.sub(r"\b(\d+\s?(mg|mcg|ml))\b", "", name, flags=re.IGNORECASE).strip().lower()

def run(state: BodyState) -> BodyState:
    vector = embed([state["user_query"]])[0]
    body = {
        "knn": {"field": "embedding", "query_vector": vector, "k": 8, "num_candidates": 50},
    ...
    for m in hits:
        ent = m.get("entity")
        base = (m.get("normalized") or {}).get("ingredient") or _base_name(m.get("name",""))
        key = (ent, base)
        if key not in seen:
